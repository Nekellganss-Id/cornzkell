...
faketeks = '©Created By Why Nekell?🔥'
blocked = []
multi =false
offline = false
nopref = false
...
success: 'Done Idol... Jangan Lupa Bahagia #WhyNekell🔥',
...
status: 1,
surface : 1,
message: `Follow Fb : Nekell Ganss`, 
orderTitle: `Facebook : Nekell Ganss`,
thumbnail: fakethumb,
...
「 INFO BOT 」
🎃Creator Bot : '©Created By Why Nekell?🔥'
🌐 Prefix :  ⌜  ${prefix}  ⌟
🍬 Name Bot : ${NameBot}
...
sendButLocation(from, menu, "©Created By Why Nekell?🔥", fakeimg, but, zer)
break
case 'downloadmenu':
menu = `❏ 「 \`\`\`DOWNLOAD MENU\`\`\` 」
...
var btngrass = {
contentText: `${anu}`,
footerText: 'Created By Why Nekell?🔥',
buttons: punten,
headerType: 1
}
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `Jangan Lupa Follow Facebook : Nekell Ganss`)
fs.unlinkSync(ran)
})
break
...
case 'facebook':
teks =
`Nih Facebook Owner Ku Jangan Lupa Di Add Ya https://www.facebook.com/nekellganss`
zero.sendMessage(from, teks, text, {quoted: ftrol})
break
case 'instagram':
teks =
...
𝐂𝐫𝐞𝐚𝐭𝐨𝐫 𝐒𝐜𝐫𝐢𝐩𝐭 : NekellGanss
𝐋𝐢𝐧𝐤 𝐒𝐜𝐫𝐢𝐩𝐭? YAHAHAH MAKA NYA USAHA KONTOL, MAU NYA DI SUAPIN MULU GOBLOK! CEK YOUTUBE BANYAK SC TAPI YA DI ENC SEMUA AOWKWOWK`
...
*┃◗ Thank To Ortu*
*┃◗ Thank To Why Nekell?🔥 (Creator)*
*┃◗ Thanks To My Friend&My Idoll!*
*┗━━━━━━━ •*`
                  but = [
...
await zero.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: ftrol, caption: 'Nihh Cok #WhyNekell?🔥'})
break
case 'ytmp3':
if (args.length === 0) return reply(`Link Nya Mana ?\nContoh: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
...
var ZeroYT7 = {
           ...
           zero.sendMessage(from, rules, MessageType.text, WhyNekell)
           break 
case 'tag':
...
case 'codwarzone':
case 'valorantbanner':
if (args.length == 0) return reply(`Teks Nya Mana ?\nContoh : ${prefix + command} WhyNekell`)
bo = args.join(" ")
reply(mess.wait)
anu = await getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey1}&text=${bo}`)
zero.sendMessage(from, anu, image, {quoted: ftrol, caption: 'Done Cokk #WhyNekell?🔥' })
break
case 'hartatahta':
case 'naruto':
case 'bneon':
...
zero.sendMessage(from, res, image, {quoted: ftrol, caption: 'Done Cokk #WhyNekell?🔥' })
break
case 'wolflogo':
case 'pubglogo':
case 'snowwrite':
...
zero.sendMessage(from, wll, image, {quoted: ftrol, caption: 'Done Cokk #WhyNekell?🔥' })
break
case 'blackpink':
case 'neon':
case 'greenneon':
...
zero.sendMessage(from, blck, image, {quoted: ftrol, caption: 'Done Cokk #WhyNekell?🔥' })
break
case 'ssweb':
...
zero.sendMessage(from, hasil, image, { thumbnail: Buffer.alloc(0), caption: `Done Mekk Jangan Lupa Follow Fb : Nekell Ganss`, quoted : ftrol})
})
break
case 'asupan':
...
zero.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Nih Cokk #WhyNekell?🔥'})
break
case 'asupancecan':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=${apikey5}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
zero.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Nih Cokk #WhyNekell?🔥'})
break
case 'asupanhijaber':
